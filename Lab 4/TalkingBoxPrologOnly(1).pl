...
validate_and_query_options(L):-
	findnsols(100,X,like(X),Likelist),print(Likelist),findnsols(100,X,dislike(X),Dislikelist),print(Dislikelist),append(Likelist,Dislikelist,History), list_to_set(L,S), list_to_set(History,H), subtract(S,H,Valid),print(Valid),member(X,Valid), print(X), print('? y/n/q: '), read(Like), (Like==q -> abort;Like==y -> assert(like(X));assert(dislike(X))), ask(X).
