...
:- dynamic answered/1, have_symptom/1.
...
nextQuestion(Next):-
    ...
    (
        (current_predicate(pain/1);If_pain_finished),   % if one of pain is selected or Pain_level is answered through
        (current_predicate(mood/1);If_mood_finished)    % if one of mood is selected or Mood_level is answered through
    ),!,
...
answer(Q, Answer):-
	...
	diagnose(Result),
	(same(Result) -> assert(ready_to_diagnose(true));true),
        (If_symptom_finished -> assert(ready_to_diagnose(true));true)
    ).
